...
| `/` | Standard View (overview dashboard) | Partially built — Source Terminal, Heat Map, and RES Map panels (top row, in that order) all have real widget content (live map + energy-infrastructure overlay, usage bars, map controls); General Source Terminal and Usage Terminal (bottom row) still show placeholders |
...
**Source Terminal / Heat Map / RES Map** (`src/components/panels/EnergyMapPanel.tsx`, in that left-to-right order in the grid) all render the exact same widget set through one shared, parameterized component — built from the pulled Figma design, since refined heavily against the running app: a real **Leaflet** map (`EnergyMap.tsx`, `react-leaflet` + `leaflet`) using CARTO's free "Positron" tile set (light/muted OpenStreetMap-data basemap, no API key needed — replaced the original static Figma export image and hex-mesh overlay), a segmented usage-bars widget (`UsageBarsWidget.tsx`, top segment of each column has a subtle opacity-flicker animation), map controls (zoom/locate/move, fullscreen, collapse — `MapControls.tsx`, currently decorative, not wired to the Leaflet instance yet), and the standard `PanelHeaderBadge` (matching every other panel header in the app, not a bespoke style). Each panel just passes a different `title`/`center`/`zoom`/`bbox` (`src/lib/mapLocations.ts`) to real Austrian locations in the Leibnitz district, Styria: **Source Terminal** shows the whole district (zoom 12), **Heat Map** is focused on Leibnitz town center (zoom 14), **RES Map** is focused on Kaindorf an der Sulm (zoom 14) — coordinates looked up via Nominatim (OSM's free geocoder, no key).

**Live energy infrastructure overlay** (`src/lib/energyInfrastructure.ts`): queries the public **Overpass API** (`overpass-api.de`, OpenStreetMap data, no key) for every `power=plant` / `generator` / `substation` / `transformer` inside each panel's own bounding box, and renders each as a colored `CircleMarker` (color by source: solar/wind/hydro/gas/biomass/other-generation/substation/transformer — see `energySourceColor`) with a **hover tooltip** (not a click popup) showing its name and type. The free Overpass instance 429/503/504s under load fairly often (happened repeatedly during dev testing — it's the public server straining, not a code bug), so the fetch: retries up to twice with backoff (1.5s, 4s) on retryable statuses, and caches successful results in `localStorage` for 1 hour keyed by bbox (so reloads are instant and don't re-hit Overpass — each panel's bbox is cached separately). While loading/erroring, a small status pill shows at the top of the panel ("Loading energy sources…" / "Energy sources unavailable" with a Retry button) instead of silently showing nothing. Both `EnergyMap`'s `MapContainer` and the outer `EnergyMapPanel` (`PanelShell`) carry `isolate` (CSS `isolation: isolate`) — Leaflet's internal panes use fixed z-index values (200–700) that leak out and compete with the rest of the page if nothing establishes a stacking context, and each panel's own `z-[1000]` header/controls (needed to sit above the map) do the same thing one level up if the panel itself isn't isolated too. Without both, either the map or the panel's header/controls could render on top of unrelated UI elsewhere on the page (e.g. TopBar popups) — isolating only one level isn't enough, both are needed. Tight zoom (e.g. Heat Map at 14) means some of a panel's bbox extends past what's visible on load — infrastructure tends to sit on a town's outskirts, not its main square, so don't be surprised if a panel looks sparse until you pan. Dense areas can still return a lot of `transformer` points (Austria's OSM power-grid mapping is very thorough) — worth revisiting with clustering (e.g. `react-leaflet-cluster`) or a category toggle/legend if a panel ends up feeling too busy.
...
- Source Terminal / Heat Map / RES Map are real Leaflet/OpenStreetMap maps now, but the zoom/locate/move/fullscreen/collapse control buttons are still decorative — not wired to the map instance (users can pan/scroll-zoom the map directly in the meantime).
- The energy-infrastructure overlay hits the public Overpass API directly from the client (with retry + 1h localStorage caching now) — fine for a prototype, but worth a real backend proxy + cache (and possibly marker clustering) before this goes anywhere real. Three panels now query independently on first load (each with its own bbox), so cold-load traffic to Overpass tripled — worth watching if rate limits become a recurring annoyance.
